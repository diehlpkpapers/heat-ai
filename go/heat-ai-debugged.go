...
func main() {
	...
	for i := 0; i < Worker; i++ {
        ...
        if start == 0 { start =  1} // Start to compute the stenctil at 1 to have a left neighbor
        if stop > Nx { stop = Nx } // Compute the stencil t the second last point to a have a right neighbor
